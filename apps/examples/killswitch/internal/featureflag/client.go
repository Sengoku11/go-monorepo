...
// NewClient returns an instantiated fflag.Client using Go Feature Flag in-process provider.
//
...
func NewClient(ctx context.Context, log logger.Logger) (*fflag.Client, error) {
	...
	client, err := fflag.New(namespace, provider, log)
	if err != nil {
